...
testSuites(
  testPrefix,
  "https://comments-benchmark.pages.dev/social",
  testTypes,
);
...
function testSuites(type: string, prefix: string, names: string[]) {
  range(amountOfRuns).forEach((i) =>
    names.forEach((name: string) =>
      test(prefix + " - " + name + " audit social #" + (i + 1), () =>
        auditSocialPage(type, prefix, name, i + 1),
      ),
    ),
  );
}

async function auditSocialPage(
  type: string,
  prefix: string,
  name: string,
  n: number,
...
function getReportsConfiguration(prefix: string) {
  return {
    ...
    name: prefix + "-audit",
    directory: `test-output/social-benchmark/${FORM_FACTOR}`,
